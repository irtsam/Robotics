...
class GetRoutes(handler.ContentHandler):
  ...
  def printRoute(self,item):
    ...
    #Route having lat and long saved in text file
...
data = LoadOsm("giki.osm")
node1 = data.findNode(float(sys.argv[2]),float(sys.argv[3]),'car')
node2 = data.findNode(float(sys.argv[4]),float(sys.argv[5]),'car')
print("NODE 1 : %d" %node1)
print("NODE 2 : %d" %node2)
obj.doRouting(int(node1), int(node2))
